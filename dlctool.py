...
def read_lines_with_timeout(
    file: io.IOBase, timeout: float, maxlines: int = 4
) -> List[str]:
    """
    ...
    def run(self) -> Set[Tuple[str, str, str]]:
        ...
        if proc.returncode != 0 and not is_timeout:
            if "Operation not permitted" in stdout:
                sys.stderr.write(
                    f"rc:{proc.returncode} 权限不足：尝试使用 sudo 或以 root 身份运行脚本"
                )
            else:
...
class GDBController:
    def __init__(
        ...
        gdbserver_path="/usr/local/gdbserver",
    ):
        self.host = host
        self.port = port
        self.pid = pid
        ...
        try:
            ...
            )  # 远程连接超时10s, 才会输出信息，这里设置15s以捕获输出
            if "error" in res:
                sys.stderr.write(
                    f"Error connecting to {host}:{port}, Error message: {res}\n"
                )
                sys.exit(1)
    ...
    def _start_server(self, timeout: float = 3.0) -> Tuple[bool, str]:
        ...
        try:
            ...
            sys.stderr.write(
                f"GDBServer error for {self.pid}: {e.output}\n"
            )  # 将错误信息输出到标准错误
            sys.exit(1)
        except Exception as e:
            sys.stderr.write(f"Error starting gdb: {e}\n")
        ...
        # 检查输出中是否包含权限错误
        if any(
            re.search(pattern, output, re.IGNORECASE) for pattern in permission_errors
        ):
            advice = (
                "\n[解决方案建议]\n"
...
class DeadLockChecker:
    def __init__(
        ...
        stack_info: Dict[str, Dict[str, List[str]]],
    ):
        self.data = data
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--host", type=str, default="localhost", help="gdbserver host address"
    )
    parser.add_argument("--port", type=int, default=12345, help="gdbserver port")
    parser.add_argument(
        "-s", "--symbol-file", type=str, default=None, help="Use FILE as symbol file"
    )
    parser.add_argument(
...
def main():
    ...
    if args.pid is None:
        ...
        if not pid:
            sys.stderr.write(
                f"Error: Could not find process with executable name {args.executable}\n"
            )
            sys.exit(1)
